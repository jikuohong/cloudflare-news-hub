...
| 🌤 | **天气推送** | 可自定义推送时间，推送城市天气，含逐时预报与未来两天，无需 API Key |
...
> 每分钟触发一次。系统根据当前时间决定执行内容：到达配置的推送时间点时推送天气/新闻，其余时间仅检查失败重试，几乎不消耗资源。免费套餐每天 10 万次请求，每分钟触发一天仅 1440 次，完全够用。
...
在侧边栏推送设置中配置城市和推送时间，所有已配置渠道同步接收。
...
2. 填写推送时间（支持 `HH:MM` 格式，逗号分隔多个，默认 `7:30`）
3. 开启「启用天气推送」开关
4. 点击 **💾 保存配置**
5. 可点击 **⛅ 测试天气推送** 立即验证
...
| `7:30` | 每天早上 7:30 |
| `17:00` | 每天下午 5 点整 |
| `7:30,17:00` | 早上 7:30 和下午 5 点各推一次 |
| `8` | 等同于 `8:00`，兼容纯小时格式 |

> 新闻推送时间格式完全相同，两者可独立配置。
...
<details>
<summary><b>⑦ 分钟级精准推送</b></summary>

推送时间支持 `HH:MM` 格式（如 `7:30`、`12:00`），Cron 每分钟触发一次，系统检测当前时间是否在设定时间点的 ±2 分钟窗口内，命中后写入 KV 防止重复推送。天气与新闻推送时间相互独立，可分别配置。
...
| `lastRun_<date>_<hour>_<minute>` | 当天当时间点新闻是否已推标记 | 24 小时 |
| `push_failed_channels` | 新闻推送失败的渠道列表 | 2 小时 |
| `weather_lastRun_<date>_<hour>_<minute>` | 当天当时间点天气是否已推标记 | 24 小时 |
...
<details>
<summary><b>天气/新闻推送时间怎么填？</b></summary>

支持 `HH:MM` 格式，逗号分隔多个时间点：

- 早上 7 点半 → `7:30`
- 下午 5 点 → `17:00`
- 多个时间 → `7:30,12:00,17:00`
- 纯小时也兼容 → `8` 等同于 `8:00`
